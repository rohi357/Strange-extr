...
async def decrypt_url(encrypted_str: str) -> str:
    ...
    if not encrypted_str.startswith(f"{CREDIT}_"):
        return encrypted_str
        
    try:
        # Remove prefix and decode base64
        encrypted_data = base64.b64decode(encrypted_str[len(CREDIT)+1:])
...
async def process_file_content(content: str, encrypt: bool = True) -> str:
    ...
    try:
        ...
        #url_pattern = rf'(?:https://[^\s\n:]+|{re.escape(CREDIT)}_[\w+/=]+)'
        url_pattern = rf'(?:https://[^\s\n:]+|{re.escape(CREDIT)}_[a-zA-Z0-9+/=]+)'
        ...
        for url in urls:
            try:
                if encrypt:
                    processed_url = await encrypt_url(url) if not url.startswith(f"{CREDIT}_") else url
                else:
                    processed_url = await decrypt_url(url) if url.startswith(f"{CREDIT}_") else url
